...
class DressingX(AssistiveEnv):
    def __init__(self, robot_type='pr2', human_control=False):
        super(DressingX, self).__init__(robot_type=robot_type, task='dressing', human_control=human_control, frame_skip=10, time_step=0.01, action_robot_len=7, action_human_len=(10 if human_control else 0), obs_robot_len=24+5, obs_human_len=(28 if human_control else 0))
    ...
    def step(self, action):
        ...
        info = {'total_force_on_human': total_force_on_human, 'task_success': int(self.task_success >= self.config('task_success_threshold')), 'action_robot_len': self.action_robot_len, 'action_human_len': self.action_human_len, 'obs_robot_len': self.obs_robot_len, 'obs_human_len': self.obs_human_len, 'cost': -preferences_score}
        done = False

        obs = np.concatenate([obs,[distance_along_forearm], [distance_along_upperarm], [distance_to_hand], [reward_dressing], [preferences_score]])
    ...
    def reset(self):
        ...
        return np.concatenate([orig, [distance_along_forearm], [distance_along_upperarm], [distance_to_hand], [0], [preferences_score]])
