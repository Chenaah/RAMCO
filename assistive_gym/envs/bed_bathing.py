...
class BedBathingX(AssistiveEnv):
    def __init__(self, robot_type='pr2', human_control=False):
        super(BedBathingX, self).__init__(robot_type=robot_type, task='bed_bathing', human_control=human_control, frame_skip=5, time_step=0.02, action_robot_len=7, action_human_len=(10 if human_control else 0), obs_robot_len=24+3, obs_human_len=(28 if human_control else 0))
    ...
    def step(self, action):
        ...
        info = {'total_force_on_human': total_force_on_human, 'task_success': int(self.task_success >= (self.total_target_count*self.config('task_success_threshold'))), 'action_robot_len': self.action_robot_len, 'action_human_len': self.action_human_len, 'obs_robot_len': self.obs_robot_len, 'obs_human_len': self.obs_human_len, 'cost': -preferences_score}
        done = False

        obs = np.concatenate([obs, [reward_distance], [reward_new_contact_points], [preferences_score]])
    ...
    def reset(self):
        ...
        end_effector_velocity = np.linalg.norm(p.getLinkState(self.tool, 1, computeForwardKinematics=True, computeLinkVelocity=True, physicsClientId=self.id)[6])
        preferences_score = self.human_preferences(end_effector_velocity=end_effector_velocity, total_force_on_human=total_force_on_human, tool_force_at_target=tool_force_on_human)
        obs = np.concatenate([orig, [reward_distance], [reward_new_contact_points], [preferences_score]])
