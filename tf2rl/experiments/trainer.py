...
from tf2rl.experiments.utils import save_path, frames_to_gif, frames_to_mp4
...
if tf.config.experimental.list_physical_devices('GPU'):
    for cur_device in tf.config.experimental.list_physical_devices("GPU"):
        ...
        # tf.config.experimental.set_virtual_device_configuration(cur_device, [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=1024*3)])
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:
            ...
            if done or episode_steps == self._episode_max_steps:
                ...
                self.logger.info("Total Epi: {0: 5} Steps: {1: 7} Episode Steps: {2: 5} Return: {3: 6.4f} Cost: {4: 5.4f} FPS: {5:5.2f}".format(
                    n_episode, total_steps, episode_steps, episode_return, episode_cost, fps))
                tf.summary.scalar(
                ...
                wandb.log({'Training_Return': episode_return, 'Training_Cost': episode_cost, 'Cost_Rate': cost_rate, 'FPS': fps}, step=n_episode)
            ...
            if total_steps % self._test_interval == 0:
                ...
                avg_test_return, avg_test_cost = self.evaluate_policy(total_steps)
                self.logger.info("Evaluation Total Steps: {0: 7} Average Reward {1: 6.4f} Average Cost {2: 5.4f} over {3: 2} episodes".format(
                    total_steps, avg_test_return, avg_test_cost, self._test_episodes))
                wandb.log({'Evaluation_Return': avg_test_return, 'Evaluation_Cost': avg_test_cost}, step=n_episode)
