...
class AntX(WalkerBase):
  ...
  def calc_state(self):
    ...
    more = np.array(
        [
            z - self.initial_z,start_pos_xstart_pos_x
            np.sin(angle_to_target),
            np.cos(angle_to_target),
            0.3 * vx,
            0.3 * vy,start_pos_x
    ...
    etc = np.clip(np.concatenate([more] + [j] + [self.feet_contact]), -5, +5) 
    return np.concatenate([self.body_xyz[:2], etc])
    # return np.clip(np.concatenate([more] + [j] + [self.feet_contact]), -5, +5)
