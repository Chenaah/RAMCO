...
def generate_training_data_inputs(states0, controls0):
	...
	#print (f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!len of state: {len(states)}') #DEBUGGING
	for i in range(len(states)):
		curr_item = states[i]  # a rollout
		length = curr_item.shape[0]
		new_states.append(curr_item[0:length-1,:]) #(curr_item[0:length-1,:])
		#print('DEBUGGING', np.shape(new_states))

		curr_item = controls[i]
		length = curr_item.shape[0]
		new_controls.append(curr_item[0:length-1,:])  #(curr_item[0:length-1,:])
...
def from_observation_to_usablestate(states, which_agent, just_one):
	...
	if(which_agent==1):
		if(just_one):
			...
			#print ("AAAAA", np.shape(curr_item[0:29]))
			return curr_item[0:29]
			#return curr_item
			...
			for i in range(len(states)): #for each rollout
				...
				new_states.append(curr_item[:,0:29])
				#new_states.append(curr_item)
			#print ("BBBBB", np.shape(new_states))
			return new_states
